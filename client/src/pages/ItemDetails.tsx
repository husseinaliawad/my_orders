...
export function ItemDetails() {
  ...
  return <PageContainer>
    <div className="grid gap-8 lg:grid-cols-[1.15fr_.85fr]"><div><img src={assetUrl(item.images?.[0])} className="h-[360px] w-full rounded-3xl object-cover shadow-soft sm:h-[460px]" /><div className="mt-4 grid grid-cols-4 gap-3">{item.images.map((img) => <img key={img} src={assetUrl(img)} className="h-20 rounded-2xl object-cover sm:h-24" />)}</div></div>
