  ...
  return <header className="sticky top-0 z-40 border-b border-slate-200/70 bg-white/75 backdrop-blur-2xl">
    <div className="mx-auto flex max-w-[96rem] items-center justify-between px-4 py-3 sm:px-6 lg:px-8">
      ...
      <div className="hidden items-center gap-3 lg:flex">
        ...
        {user ? <div className="relative"><button onClick={() => setProfileOpen(!profileOpen)} className="flex items-center gap-2 rounded-full px-3 py-2 text-sm font-bold text-slate-700 hover:bg-slate-100"><UserRound size={17} />{user.name}</button>{profileOpen && <div className="absolute right-0 mt-2 w-48 rounded-2xl border border-slate-200 bg-white p-2 shadow-xl"><Link className="block rounded-xl px-3 py-2 text-sm font-bold hover:bg-slate-50" to="/profile">Profile</Link><button className="w-full rounded-xl px-3 py-2 text-left text-sm font-bold text-rose-600 hover:bg-rose-50" onClick={logout}>Logout</button></div>}</div> : <Link to="/login"><Button className="px-4 py-2 text-sm">Login</Button></Link>}
      </div>
      <button className="rounded-xl border border-slate-200 p-2 lg:hidden" onClick={() => setOpen(!open)}>{open ? <X /> : <Menu />}</button>
    </div>
    {open && <div className="border-t border-slate-200 bg-white p-4 lg:hidden"><nav className="grid gap-2">{links.map(navLink)}{user?.role === "admin" && navLink(["Admin", "/admin", BarChart3])}<Link className="inline-flex items-center gap-2 rounded-full px-3.5 py-2 text-[13px] font-bold text-slate-600" to="/cart"><span className="relative"><ShoppingCart size={15} />{cartBadge}</span>Cart</Link>{user ? <button onClick={logout} className="text-left rounded-full px-3.5 py-2 text-[13px] font-bold text-rose-600">Logout</button> : <Link to="/login" className="rounded-full px-3.5 py-2 text-[13px] font-bold text-primary">Login</Link>}</nav></div>}
  </header>;
}
